...
docker compose up -d            # postgres, redis, chromadb, minio, migrations,
                                # app (with migrate-gate: tables exist before
                                # the server starts), celery, flower, frontend
```
...
Health & self-diagnosis: `/health` (liveness) and `/ready` (per-dependency
checks: postgres, redis, minio, chroma, mcp_hub).
