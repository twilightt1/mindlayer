...
2. ✅ **ClawHub skill + MCP client config** — `skills/orivory/` skill package (SKILL.md runbook + tool catalog + error runbook + 3 examples; env-gated qua ORIVORY_TOKEN) + copy-paste `mcp.servers` config. (done 2026-09-04: skill package + compose migrate-gate hardening — app/celery wait for `alembic upgrade head`; publishing lên ClawHub là bước founder-side — `clawhub skill publish`; OpenClaw session-log import = follow-up)
