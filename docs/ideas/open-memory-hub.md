...
2. ✅ **ClawHub skill + MCP client config** — `skills/orivory/` skill package (SKILL.md runbook + tool catalog + error runbook + 3 examples; env-gated qua ORIVORY_TOKEN) + copy-paste `mcp.servers` config. (done 2026-09-04; publishing lên ClawHub là bước founder-side — `clawhub skill publish`; compose self-diagnosis doctor + OpenClaw session-log import = follow-ups)
