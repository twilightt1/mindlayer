...
async def run_smoke(args: argparse.Namespace) -> None:
    ...
    title = f"Orivory smoke {time.strftime('%Y-%m-%d %H:%M:%S')}"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run a production-like Orivory Demo smoke workflow.",
    )
    parser.add_argument("--base-url", default=_env("DEMO_BASE_URL", "http://localhost:8000"))
    parser.add_argument("--email", default=_env("DEMO_USER_EMAIL", "orivory-demo@example.com"))
    parser.add_argument("--password", default=_env("DEMO_USER_PASSWORD", "DemoPassword123!"))
    parser.add_argument("--display-name", default=_env("DEMO_DISPLAY_NAME", "Orivory Demo"))
